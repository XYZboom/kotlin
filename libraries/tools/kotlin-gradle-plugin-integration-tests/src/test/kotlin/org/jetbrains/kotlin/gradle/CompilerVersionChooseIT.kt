...
class CompilerVersionChooseIT : KGPBaseTest() {
    ...
    fun compilerVersionMayBeChanged(gradleVersion: GradleVersion) {
        prepareProject(gradleVersion) {
            chooseCompilerVersion("2.0.0-Beta3")
            build(printClasspathTaskName) {
                assertCompilerVersion("2.0.0-Beta3")
            }
        }
    }
    ...
    fun manualAdding(gradleVersion: GradleVersion) {
        prepareProject(gradleVersion) {
            buildGradle.append(
                ...
                """
                dependencies {
                    $KOTLIN_BUILD_TOOLS_API_CLASSPATH_CONFIGURATION_NAME("org.jetbrains.kotlin:kotlin-compiler-embeddable:2.0.0-Beta3")
                }
                """.trimIndent()
            )
            build(printClasspathTaskName) {
