...
object FirWasmJsAssociatedObjectChecker : FirBasicDeclarationChecker(MppCheckerKind.Common) {
    override fun check(declaration: FirDeclaration, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        for (annotationCall in declaration.annotations) {
            val annotation = annotationCall.annotationTypeRef.coneType.toSymbol(context.session)?.annotations ?: continue
            if (annotation.hasAnnotation(StandardClassIds.Annotations.AssociatedObjectKey, context.session)) {
